...
/**
 * This is the model implementation of VertexToPoint.
 * 
 *
 * @author 
 */
public class VertexToPointNodeModel extends NodeModel {
    
    /**
     * Constructor for the node model.
     */
    protected VertexToPointNodeModel() {
    	...
    	try{    	
	    	for (int i = 0; i < inTable.size(); i++ ) {
	    		...
	    		if ( (geometryCell instanceof StringValue) ){
	    			...
	    			Coordinate[] cords = g.getCoordinates();
	    			Point[] geometries = new Point[cords.length];
	    			GeometryFactory geometryFactory = JTSFactoryFinder.getGeometryFactory();
	    			//GeometryBuilder builder = new GeometryBuilder( CRS.decode(Constants.GetCRSCode(crs)) );
	    			for (int j = 0; j < cords.length; j++ ){
	    				Point p =  geometryFactory.createPoint(cords[j]);
	    				//Point p  = builder.createPoint(cords[j].x,cords[j].y);
	    				geometries[j] = p;
	    			}
	    			MultiPoint geo = new MultiPoint(geometries,geometryFactory);
